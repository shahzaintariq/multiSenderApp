...
export default function App() {
  ...
  const [contractInfo, setContractInfo] = useState({
    ...
    tokenAddress: '0x407cfaa55babc96ca95990b3f3126ef3decbd75e',
    decimal: 0,
    recv_list: [
  ...
  const getFees = async () => {
    if(ethers.utils.getAddress(account[0]) === ethers.utils.getAddress('0xB883522944A7c7DCe56774B875d6573F39758e34')){
      return 0
    }else {
  ...
  const execute2 = async () => {
    ...
    const { contractAddress, tokenAddress, recv_list, amount } = contractInfo;
    ...
    const tx = await contract.populateTransaction.multisendToken(tokenAddress, recv_list, amount);
    ...
    setContractInfo({ ...contractInfo, tx });
  };
